...
class UIBms(QWidget, Ui_BMS):
    ...
    #     instance = None
    #     if text == ip:
    #         if self.deviceTree.isColumnHidden(0):
    #             try:
    #                 # 向后端发送连接请求
    ...
    #
    #             self.deviceTree.setColumnHidden(0, False)
    #             root_item = QTreeWidgetItem(self.deviceTree, [str(device_id)])
    #             self.deviceTree.addTopLevelItem(QTreeWidgetItem(root_item, [str(obj_type) + ':' + str(instance)]))
    #
    #         else:
    ...
    #     item = self.deviceTree.itemAt(position)
    #     if item is not None:
    #         menu = QMenu()
    #         subscribe_action = menu.addAction("Subscribe")
    #         action = menu.exec_(self.deviceTree.viewport().mapToGlobal(position))
    #
